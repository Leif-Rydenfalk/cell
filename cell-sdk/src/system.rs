...
use cell_model::protocol::{MitosisControl, MitosisRequest, MitosisResponse, MitosisSignal};
...
impl System {
    pub async fn spawn(cell_name: &str, mut config: Option<CellInitConfig>) -> Result<String> {
        ...
        let resp: MitosisResponse = archived
    ...
    pub async fn ignite_local_cluster() -> Result<()> {
        CLUSTER_INIT
            .get_or_init(|| async {
                ...
                std::env::set_var(
                    "CELL_SOCKET_DIR",
                    target_dir.join("runtime/system").to_str().unwrap(),
                );
                std::env::set_var(
                    "CELL_REGISTRY_DIR",
                    target_dir.join("registry").to_str().unwrap(),
                );
                ...
                let (_child, mut junction) = spawn_with_gap_junction(cmd)
                    .expect("Failed to spawn Hypervisor with Gap Junction");
                ...
                let handle = std::thread::spawn(move || loop {
                    ...
                    match signal {
                        MitosisSignal::RequestIdentity => {
                            ...
                            junction
                                .send_control(MitosisControl::InjectIdentity(config))
